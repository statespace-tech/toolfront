...
pub(super) async fn run(cmd: TokensCommands, gateway: GatewayClient) -> Result<()> {
    match cmd {
...
async fn run_create(args: TokenCreateArgs, gateway: GatewayClient) -> Result<()> {
    println!("Creating token '{}'...", args.name);
...
async fn run_rotate(args: TokenRotateArgs, gateway: GatewayClient) -> Result<()> {
    println!("Rotating token {}...", args.token_id);
...
async fn run_revoke(args: TokenRevokeArgs, gateway: GatewayClient) -> Result<()> {
    if !args.yes {
        print!(
            "Are you sure you want to revoke token {}? [y/N] ",
            args.token_id
        );
        io::stdout().flush()?;
